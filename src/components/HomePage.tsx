    ...
    <div className="min-h-screen bg-white">
          ...
          <div className="flex items-center justify-between bg-white/80 backdrop-blur-sm rounded-full px-6 py-3 shadow-sm border border-gray-100">
            ...
            <div className="hidden md:flex items-center space-x-8">
              <a 
                ...
                onClick={() => posthog.capture('clicked_how_it_works')}
              >
                How it Works
              </a>
              <a 
                href="#pricing" 
                className="text-gray-600 hover:text-black transition-colors"
                onClick={() => posthog.capture('clicked_pricing')}
              >
                Pricing
              </a>
              <a 
                href="#faq" 
                className="text-gray-600 hover:text-black transition-colors"
                onClick={() => posthog.capture('clicked_faq')}
              >
                FAQ
              </a>
              {user ? (
                ...
                <button 
                  onClick={() => {
                    ...
                    posthog.capture('opened_sign_in');
                  }}
      ...
      <section className="pt-24 md:pt-28 pb-8 px-4">
        <div className="container mx-auto max-w-3xl text-center">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-3 md:mb-5 text-[#2d3748] leading-tight px-2 sm:px-4">
            Connect With The Perfect<br />Influencer/Brand...
          </h1>
          <p className="text-lg md:text-xl text-gray-600 mb-8">
            Connect... Collabrate... Grow...
          </p>
          <div className="flex flex-col space-y-3 md:space-y-0 md:flex-row justify-center md:gap-3 mb-10 px-6 md:px-0">
            <button 
              className="w-full md:w-auto bg-black text-white px-8 py-3.5 md:py-3 rounded-full text-base font-medium hover:bg-black/90 transition-colors"
              onClick={async () => {
                if (user) {
                  ...
                  posthog.capture('clicked_influencer_cta', {
                    isLoggedIn: true,
            ...
            <button 
              className="w-full md:w-auto bg-black text-white px-8 py-3.5 md:py-3 rounded-full text-base font-medium border border-gray-200 hover:bg-black/90 transition-colors"
              onClick={async () => {
                if (user) {
                  // If user is logged in, check their onboarding status
                  const { data: profile } = await supabase
                  ...
                  posthog.capture('clicked_brand_cta', {
                    isLoggedIn: true,
                    onboardingCompleted: profile?.onboarding_completed
                  });
